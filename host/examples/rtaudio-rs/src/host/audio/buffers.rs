...
impl HostAudioBuffers {
    ...
    pub fn write_to_rt_audio_buffer(&mut self, destination: &mut [f32]) {
        // For non-interleaved buffers, channels are stored as separate arrays
        // let frames_per_channel = destination.len() / 2;
        //
        // // Split the output buffer into left and right channel slices
        // let (left_channel, right_channel) = destination.split_at_mut(frames_per_channel);
        //
