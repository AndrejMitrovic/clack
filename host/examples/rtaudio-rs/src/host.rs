...
pub struct RtAudioHost;

impl HostHandlers for RtAudioHost {
    type Shared<'a> = RtAudioHostShared;
    type MainThread<'a> = RtAudioHostMainThread<'a>;
...
impl<'a> SharedHandler<'a> for RtAudioHostShared {
...
pub struct RtAudioHostMainThread<'a> {
    /// A reference to shared host data.
    /// (this is unused in this example, but this is kept here for demonstration purposes).
    _shared: &'a RtAudioHostShared,
...
impl<'a> RtAudioHostMainThread<'a> {
    /// Initializes the main thread data.
    fn new(shared: &'a RtAudioHostShared) -> Self {
        Self {
            _shared: shared,
            plugin: None,
            timer_support: None,
...
impl<'a> MainThreadHandler<'a> for RtAudioHostMainThread<'a> {
...
pub fn run(plugin: FoundBundlePlugin) -> Result<(), Box<dyn Error>> {
    ...
    let mut instance = PluginInstance::<RtAudioHost>::new(
        |_| RtAudioHostShared::new(sender.clone()),
        |shared| RtAudioHostMainThread::new(shared),
        &plugin.bundle,
        &plugin_id,
        &host_info,
    )?;
...
fn run_gui_floating(
    mut instance: PluginInstance<RtAudioHost>,
...
impl HostLogImpl for RtAudioHostShared {
...
impl HostAudioPortsImpl for RtAudioHostMainThread<'_> {
...
impl HostNotePortsImpl for RtAudioHostMainThread<'_> {
...
impl HostParamsImplMainThread for RtAudioHostMainThread<'_> {
...
impl HostParamsImplShared for RtAudioHostShared {
